...
Display.prototype.getRandomPixel = function( v, v2 ){
	...
	var found = true
	while( this.getPixel(x,y) !== v && check++ <= 200 ){
	...
	if(found){
		...
		if( typeof v2 != 'undefined') this.pixel( x,y, v2 )
		return [x,y]
	}
	return false
}
Display.prototype.copyArray = function( arr ){
	

	
	for(var i = 0; i< arr.length; i++){
		if(typeof this.grid[i] != 'undefined' ) this.grid[i] = arr[i]
	}
...
Display.prototype.drawValues = function( ctx, size, color ){
	...
	for(var y = this.height - 1; y >= 0; --y){
		for(var x = w - 1; x >= 0; --x){
			...
			if(g[i]<0) ctx.fillRect(  x * size, y * size , size,size)
			else ctx.fillText( g[i],  x * size+1, y * size + 11 )
