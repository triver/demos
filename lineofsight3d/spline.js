...
function Spline( n, loop, x, y, radius ){
	...
	if( typeof n == 'number' && x && y && radius){
		...
		for(var i = 0; i< n;i++){
			...
			this.points.push( [ x + Math.cos(a) * radius , y + Math.sin(a) * radius ] )
		}
	}
...
Spline.prototype.setMarker = function(dist){
	...
	var offset = getOffset( this.points, dist )
...
Spline.prototype.drawMarker = function(ctx, radius, color, color2, lw){
	...
	ctx.beginPath()
	ctx.moveTo(p[0], p[1])
	ctx.lineTo( p[0] + Math.cos(a)*radius, p[1] + Math.sin(a)*radius )
...
function SplineEditor(canvas, path ){
